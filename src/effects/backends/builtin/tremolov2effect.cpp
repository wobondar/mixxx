...
namespace {
//  Asymmetric slew: a gate wants a fast attack (~3.3 ms full swing) and a
//  gentler release; the stock symmetric 0.001/frame (22.7 ms @ 44.1k)
//  works as a release but smears the attack.
constexpr double kMaxGainIncrementUp = 0.0068;
constexpr double kMaxGainIncrementDown = 0.001;
} // namespace

// static
QString TremoloV2Effect::getId() {
    return "org.mixxx.effects.tremolov2";
}

// static
EffectManifestPointer TremoloV2Effect::getManifest() {
    EffectManifestPointer pManifest(new EffectManifest());
    pManifest->setId(getId());
    pManifest->setName(QObject::tr("Tremolo V2"));
    pManifest->setShortName(QObject::tr("TremoloV2"));
    ...
    // Floor lowered 0.1 -> 0.05 so narrow stab duties (1/16 of the
    // period) are reachable.
    width->setRange(0.05, 0.5, 0.9);
...
void TremoloV2Effect::loadEngineEffectParameters(
...
void TremoloV2Effect::processChannel(
    ...
    //  V2: re-anchor the LFO phase to the beat grid EVERY buffer, not just
    //  on Enabling. The stock effect free-runs after enable, so a rate
    //  change mid-flight keeps the accumulated counter and lands the gate
    //  at a random, permanent offset from the beat (measured ~26 ms off
    //  the kick after one selector flip). Only sub-beat periods
    //  (rate >= 1) can be anchored from the within-beat fraction; longer
    //  periods keep the stock free-run + enable-anchor behavior.
    if (gf.beat_length.has_value() && gf.beat_fraction_buffer_end.has_value() &&
            rate >= 1.0 && framePerPeriod > 0) {
        const double framePerBeat =
                gf.beat_length->seconds * engineParameters.sampleRate();
        ...
        currentFrame = static_cast<unsigned int>(startFrame);
    }
