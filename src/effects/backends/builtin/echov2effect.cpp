...
constexpr int EchoV2GroupState::kMaxDelaySeconds;
...
// static
QString EchoV2Effect::getId() {
    return "org.mixxx.effects.echov2";
}

// static
EffectManifestPointer EchoV2Effect::getManifest() {
    ...
    pManifest->setName(QObject::tr("Echo V2"));
    pManifest->setShortName(QObject::tr("EchoV2"));
...
void EchoV2Effect::loadEngineEffectParameters(
...
void EchoV2Effect::processChannel(
        EchoV2GroupState* pGroupState,
    ...
    if (groupFeatures.beat_length.has_value()) {
        ...
        // V2: floor lowered 1/8 -> 1/16 (the stock clamp silently played
        // 1/8 for any shorter request) and quantize granularity refined
        // from 1/4 to 1/16 so short delays survive quantization.
