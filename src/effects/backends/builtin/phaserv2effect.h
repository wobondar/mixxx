...
class PhaserV2GroupState final : public EffectState {
  public:
    PhaserV2GroupState(const mixxx::EngineParameters& engineParameters)
            : EffectState(engineParameters) {
        clear();
    }
    ~PhaserV2GroupState() override = default;
...
class PhaserV2Effect : public EffectProcessorImpl<PhaserV2GroupState> {
  public:
    PhaserV2Effect() = default;
    ~PhaserV2Effect() override = default;
  ...
  private:
    ...
    DISALLOW_COPY_AND_ASSIGN(PhaserV2Effect);
};
